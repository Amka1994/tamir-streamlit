...
# Барааны тоо ширхэг хасах функц
def remove_product_quantity(product_id: int, remove_quantity: int, reason: str="", changed_by: str="system"):
    if remove_quantity <= 0:
        return False, "Хасах тоо ширхэг 0-ээс их байх ёстой."
    try:
        with engine.begin() as conn:
            ...
            current_qty = result[0]                    # 
            new_qty = current_qty - remove_quantity
            ...
            conn.execute(history_query, {
                "product_id": product_id,
                "change_type": "REMOVE",
                "quantity_change": remove_quantity,
                "previous_quantity": current_qty,
                "new_quantity": new_qty,
                "reason": reason or "Тоо хассан",
                "changed_by": changed_by
            })
        return True, f"Амжилттай -{remove_quantity} хасагдлаа. Шинэ тоо: {new_qty}"
    except Exception as e:
        return False, f"Алдаа гарлаа: {e}"